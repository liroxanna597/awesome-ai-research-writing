...
- **分支**：后续请**直接在 `main` 分支**上继续写作和保存，不再新建日常写作分支。
...
后续所有正文写作，请默认在 `3.18 多智能体舆情模拟/` 这个工作区继续，并且**直接保存在 `main` 分支**。除非你明确要求做实验性分支，否则不要再为日常论文修改新建分支，这样可以避免每次 push 后 GitHub 出现新的分支。
同时，不再把主稿写进模板目录。
